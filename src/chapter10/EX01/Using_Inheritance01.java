...
//		-- 부모클래스(Super Class) 의 필드와 메소드, 이너클래스를 자식 클래스에서 돌려 준다.
//		-- 자식 클래스(Child Class) 는 부모 클래스의 코드(
//		-- 중복된 코드를 방지 할 수 있다.
//		-- 코드를 간결하게 사용.
//		-- 유지 보수를 아주 쉽게 한다.
//		-- 다형성 사용 : 부모클래스의 메소드를 자식 클래스에서 재정의해서 사용
//			-- 다양한 타입으로 사용이 가능하다.

//부모 클래스(Super class) : 자식 클래스의 공통적인 특징을 가지는 클래스 

class Human{	//사람  	: 부모 클래스 : 자식클래스의 공통적인 필드와 메소드를 선언
	//필드
	String name;
	int age;
...
//자식 클래스 : 부모클래스의 필드와 메소드를 그대로 상속 받는다.
...
class Student extends Human{	//Student 클래스는 Human 클래스를 상속 받는다. 
	...
	// 자식 클래스의 특징을 가진 필드와 메소드를 선언
		...
		void goToSchool() {		//자식클래에서만 사용 가능한 메소드
			System.out.println("학생이 학교에 갑니다.");
		}
}
class Worker extends Human{	//직장인
	// 부모클래스의 필드와 메소드가 그대로 내려온다.
	int workerID;			//자식클래스에서 선언된 필드	
	void goToWork() {		//자식클래에서만 사용 가능한 메소드
		System.out.println("직장인이 일하러 갑니다.");
	}
}
class Professor extends Human{	//교수
	// 부모클래스의 필드와 메소드가 그대로 내려온다.
		int professortID;			//자식클래스에서 선언된 필드	
		void goToCollage() {		//자식클래에서만 사용 가능한 메소드
