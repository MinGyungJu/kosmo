...
public class If_01 {
	public static void main(String[] args) {
		  ...
		  제어문 : 프로그램의 순서를 바꾸는 구문
		   ...
		   if  : 조건을 만족하는 실행문을 실행 후 if 문을 빠져나온다.
		   		(break를 사용하지 않더라도 if문을 빠져나온다.
		   		 swith 문은 break를 사용해서 구문을 빠져 나온다. ) 
		   		 
	  		- if (조건) { 실행1; } ;	//조건이 참일때 실행문1 을 실행 
	  		...
	  		- if (조건1) { 			//조건1 이 참일때, 실행문 1을 실행
	  			  실행문 1; 
	  		  } else if (조건2) {		//조건2 이 참일떄, 실행문 2을 실행
	  		  	  실행문 2;
	  		  } else if (조건3) {		//조건 3이 참일때, 실행문 3을 실행
	  		  	  실행문 3;
	  		  } 
	  		  ...
		...
		//if 조건에서 실행문이 1개일떄는 중괄호를 생략 할 수 있다. (주의요함)
			//권장사항 : 중괄호 명시후 사용.
		System.out.println("=========================");
		if (5<3) 
		...
		/* 2. if (조건) { 
				  실행문1 ;			// 조건이 참일때, 실행문 1실행
	   		  	  ...
	   		  	  실행문2 ;			// 조건이 거짓일때, 실행문 2실행
	   		  }
	   		  
	    */
			
		if (5>3 ) {		// 조건은 반드시 true, false
			System.out.println("실행1 (참)");			//조건이 참일떄 실행
		}else {
		...
		//if (조건) ~ else 구문을 삼한 연산자로 변환   : 삼항 연산자는 한 라인으로 처리 
