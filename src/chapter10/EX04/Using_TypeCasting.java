...
// 다운 캐스팅 : 수동으로 직접 해야한다.
	...
	// 상속 		: 부모타입 ==> 자식 타입 , (실행시 오류가 발생 될 수 있다)
...
public class Using_TypeCasting {
	public static void main(String[] args) {
		...
		//bb는 A,B 포함하고 있고 , B타입으로 지정(A,B모두 사용됨)
		B bb = new B();
		A a = (A)bb;		// B --> A 업캐스팅 (A 필드, 메소드)
		
		//2. 다운캐스팅(수동으로 직접할당) : 캐스팅이 불가능한 경우 실행시 오류
				// 다운캐스팅이 불가한 경우 
		A aa =new A(); 	//aa 는 A타입만 내포하고 있다.
//		B b = (B)aa;	//aa 는 B타입을 내포하고 있지 않으므로 다운캐스팅을 할 수 없다.
						//구문에는 문제가 없고, 실행시 문제(예외) 발생 ,
//		C c = (C)aa;	//aa는 C타입을 내포하고 있지 않아서 다운캐스팅이 불가함.
						//구문에는 문제가 없다, 실행시 오류 발생
		
		//3. 다운 캐스팅 가능한 경우 
	
		A ab = new B();	//ab는 A, B 를 내포하고 있다, A타입으로 지정됨
		B b = (B)ab; 	//A(부모) --> B(자식) 다운캐스팅
//		C c = (C)ab;	//ab 는 C타입을 내포하고 있지 않음. 캐스팅이 불가능
						//구문에는 문제없다, 실행시 오류 발생
		
		B bd = new D();	//bd는 A,B,D 를 내포하고 있고, B타입으로 지정
						//bd 는 A,B 의 필드와 메소드를 접근
		D d = (D)bd;	//bd는 B 를 내포하므로 B->D //다운캐스팅
						//bd 는 A,B,D 의 필드와 메소드를 사용가능
