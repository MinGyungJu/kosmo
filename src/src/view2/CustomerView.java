...
public class CustomerView extends JPanel 
{
	...
	public void registCustomer(){
		...
		String name = tfCustName.getText(); 	//  텍스트필프고객이름값을 가져와서 각각의 변수에 저장
		...
		CustomerVO vo = new CustomerVO(); 	// vo 지정 
		vo.setName(name); // vo에 세팅
		vo.setAddr(Addr);
		vo.setEmail(Email);
		vo.setTel(Tel2);
		vo.setTel2(Tel2);
		//생성자로 값 지정 : CustomerVO vo = new CustomerVO(name,~~)
	...
	public void connectDB(){
		...
		try {
			model = new CustomerDaoImpl(); 	// model 커넥트
		} catch (Exception e) {
